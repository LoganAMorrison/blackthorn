// Addapted from GSL's qng fixed-order quadrature.
...
#ifndef BLACKTHORN_SPECTRA_QUAD_H
#define BLACKTHORN_SPECTRA_QUAD_H
...
enum class FixedGaussKronrodCode {
  InvalidTol,
  DidNotConverge,
  Success,
};

struct FixedGaussKronrodResult { // NOLINT
  double result;
  double abserr;
  size_t neval;
  FixedGaussKronrodCode code;
};

class FixedGaussKronrod {

  // x1, abscissae common to the 10-, 21-, 43- and 87-point rule
  static constexpr std::array<double, 5> x1 = {
  ...
  static constexpr std::array<double, 5> w10 = {
  ...
  static constexpr std::array<double, 5> x2 = {
  ...
  static constexpr std::array<double, 5> w21a = {
  ...
  static constexpr std::array<double, 6> w21b = {
  ...
  static constexpr std::array<double, 11> x3 = {
  ...
  static constexpr std::array<double, 10> w43a = {
  ...
  static constexpr std::array<double, 12> w43b = {
  ...
  static constexpr std::array<double, 22> x4 = {
  ...
  static constexpr std::array<double, 21> w87a = {
  ...
  static constexpr std::array<double, 23> w87b = {
  ...
  static auto rescale_error(double err, double res_abs, double res_asc)
      -> double;

public:
  template <class F>
  static auto qk(F f, double a, double b, double epsabs, double epsrel)
      -> FixedGaussKronrodResult;
};

auto FixedGaussKronrod::rescale_error(double err, double res_abs, // NOLINT
                                      double res_asc) -> double {
...
template <class F>
auto FixedGaussKronrod::qk(F f, double a, double b, double epsabs, // NOLINT
                           double epsrel) -> FixedGaussKronrodResult {
  static constexpr size_t FV_SIZE = 5;
  static constexpr size_t SAVE_SIZE = 21;

  static constexpr size_t NEVAL_10 = 10;
  static constexpr size_t NEVAL_21 = 21;
  static constexpr size_t NEVAL_43 = 43;
  static constexpr size_t NEVAL_87 = 87;

  std::array<double, FV_SIZE> fv1{};
  std::array<double, FV_SIZE> fv2{};
  std::array<double, FV_SIZE> fv3{};
  std::array<double, FV_SIZE> fv4{};
  // array of function values which have been computed
  std::array<double, SAVE_SIZE> savfun{};
  double res10 = 0.0;
  double res21 = 0.0;
  double res43 = 0.0;
  double res87 = 0.0;
  ...
  static constexpr double EPSREL_THRES =
      50 * std::numeric_limits<double>::epsilon();
  static constexpr double EPSREL_MIN = 0.5e-28;

  if (epsabs <= 0 && (epsrel < EPSREL_THRES || epsrel < EPSREL_MIN)) {
    return FixedGaussKronrodResult{0.0, 0.0, 0,
                                   FixedGaussKronrodCode::InvalidTol};
  };

  // Compute the integral using the 10- and 21-point formula.

  res10 = 0;
  res21 = w21b.back() * f_center;
  resabs = w21b.back() * fabs(f_center);
#pragma unroll 5
  for (size_t k = 0; k < x1.size(); ++k) {
    const double abscissa = half_length * x1[k]; // NOLINT
    const double fval1 = f(center + abscissa);
    const double fval2 = f(center - abscissa);
    const double fval = fval1 + fval2;
    res10 += w10[k] * fval;                          // NOLINT
    res21 += w21a[k] * fval;                         // NOLINT
    resabs += w21a[k] * (fabs(fval1) + fabs(fval2)); // NOLINT
    savfun[k] = fval;                                // NOLINT
    fv1[k] = fval1;                                  // NOLINT
    fv2[k] = fval2;                                  // NOLINT
  }
...
#pragma unroll 5
  for (size_t k = 0; k < x2.size(); ++k) {
    const double abscissa = half_length * x2[k]; // NOLINT
    const double fval1 = f(center + abscissa);
    const double fval2 = f(center - abscissa);
    const double fval = fval1 + fval2;
    res21 += w21b[k] * fval;                         // NOLINT
    resabs += w21b[k] * (fabs(fval1) + fabs(fval2)); // NOLINT
    savfun[k + 5] = fval;                            // NOLINT
    fv3[k] = fval1;                                  // NOLINT
    fv4[k] = fval2;                                  // NOLINT
  }
...

  {
    ...
    resasc = w21b.back() * fabs(f_center - mean);

#pragma unroll 5
    for (size_t k = 0; k < w21a.size(); ++k) {
      const double r1 = resasc +=
          (w21a[k] *                                                   // NOLINT
               (std::abs(fv1[k] - mean) + std::fabs(fv2[k] - mean)) +  // NOLINT
           w21b[k] *                                                   // NOLINT
               (std::fabs(fv3[k] - mean) + std::fabs(fv4[k] - mean))); // NOLINT
    }
  ...
  if (err < epsabs || err < epsrel * fabs(result_kronrod)) {
    return FixedGaussKronrodResult{result_kronrod, err, NEVAL_21,
                                   FixedGaussKronrodCode::Success};
  }

  // compute the integral using the 43-point formula.

  res43 = w43b.back() * f_center;

#pragma unroll 10
  for (size_t k = 0; k < w43a.size(); ++k) {
    res43 += savfun[k] * w43a[k]; // NOLINT
  }

#pragma unroll 11
  for (size_t k = 0; k < x3.size(); ++k) {
    const double abscissa = half_length * x3[k]; // NOLINT
    const double fval = (f(center + abscissa) + f(center - abscissa));
    res43 += fval * w43b[k]; // NOLINT
    savfun[k + 10] = fval;   // NOLINT
  }
  ...
  if (err < epsabs || err < epsrel * fabs(result_kronrod)) {
    return FixedGaussKronrodResult{result_kronrod, err, NEVAL_43,
                                   FixedGaussKronrodCode::Success};
  }

  // compute the integral using the 87-point formula.

  res87 = w87b.back() * f_center;

#pragma unroll 21
  for (size_t k = 0; k < w87a.size(); ++k) {
    res87 += savfun[k] * w87a[k]; // NOLINT
  }

#pragma unroll 22
  for (size_t k = 0; k < x4.size(); ++k) {
    const double abscissa = half_length * x4[k];                      // NOLINT
    res87 += w87b[k] * (f(center + abscissa) + f(center - abscissa)); // NOLINT
  }
  ...
  if (err < epsabs || err < epsrel * fabs(result_kronrod)) {
    return FixedGaussKronrodResult{result_kronrod, err, NEVAL_87,
                                   FixedGaussKronrodCode::Success};
  }

  // failed to converge
  return FixedGaussKronrodResult{result_kronrod, err, NEVAL_87,
                                 FixedGaussKronrodCode::DidNotConverge};
}

} // namespace blackthorn

#endif // BLACKTHORN_SPECTRA_QUAD_H
