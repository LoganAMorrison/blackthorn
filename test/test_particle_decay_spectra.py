...
from hazma import positron_spectra
...
def test_dnde_muon_photon_rest_frame():
    ...
    dnde_hazma = decay.muon(es_hazma, parameters.muon_mass) * 1e3
...
def test_dnde_muon_photon_boosted():
    ...
    dnde_hazma = decay.muon(es_hazma, emu_mev) * 1e3
...
def test_dnde_muon_positron_rest_frame():
    ...
    es_hazma = xs * parameters.muon_mass / 2.0
    dnde_hazma = positron_spectra.muon(es_hazma, parameters.muon_mass) * 1e3
...
def test_dnde_muon_positron_boosted():
    ...
    es_hazma = xs * emu_mev / 2.0
    dnde_hazma = positron_spectra.muon(es_hazma, emu_mev) * 1e3
...
def test_dnde_charged_pion_photon_rest_frame():
    ...
    dnde_hazma = decay.charged_pion(es_hazma, epi_mev) * 1e3
