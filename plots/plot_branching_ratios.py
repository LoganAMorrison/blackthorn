...
def make_mass_branching_fraction_dict(datafile, mev=True):
    if mev:
        key = "mev"
        catagorizor = branching_fraction_catagorized_mev
    else:
        key = "gev"
        catagorizor = branching_fraction_catagorized_gev
    ...
    with h5py.File(datafile, "r") as f:
        for gen in gens:
            gstr = gen_to_str(gen)
            data = f[key][gstr]  # type: ignore
            mns[gen]: RealArray = data["masses"][:]  # type: ignore
            brs_raw: Dict[str, RealArray] = data["branching_fractions"]  # type: ignore
            brs[gen] = catagorizor(brs_raw, gen, (len(mns[gen]),))
    return (mns, brs)


def plot_branching_ratios(datafile, outfile):
    gens = [Gen.Fst, Gen.Snd, Gen.Trd]

    _, axes = plt.subplots(
        nrows=2, ncols=3, dpi=150, sharex=False, sharey=False, figsize=(12, 8)
    )

    for i, k in zip([0, 1], ["mev", "gev"]):
        mns, brs = make_mass_branching_fraction_dict(datafile, mev=(k == "mev"))
        axes_ = axes[i]
        for (gen, ax) in zip(gens, axes_):
            config = plot_config[k][gen]
            ycutoff = min(config["ylims"])

            for key, val in brs[gen].items():
                if np.max(val) > ycutoff:
        ...
        axes_[0].set_ylabel(r"$\mathrm{Branching} \ \mathrm{Ratio}$", fontsize=16)
...
if __name__ == "__main__":
    ...
    plot_branching_ratios(datafile, figures_dir.joinpath("brs_mev_gev_all.pdf"))
